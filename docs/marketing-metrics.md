...
| When (CDMX) | Job | Notes |
|---|---|---|
| ticks at minute 5–14, 20–29, 35–44, 50–59 | `leadLinkSweep` (≤40), `studentLinkSweep` (≤5) | links leads → Día/Mes/Anuncio and students → Lead. No cursor: linked rows drop out of the filter, so a failed tick just retries. The lead sweep has two passes under the one cap: leads without `Día`, then day-linked leads whose `Ad ID` arrived late (twin sweep) and still lack `Anuncio` — ≤2 lists + 1 PATCH per 10. |
...
- **2026-09-21 (late `Anuncio` links):** the ad rollups were undercounting — 71 of 2,162 ad leads since July had `Ad ID` but no `Anuncio`, and those 71 held 70 bookings (rollups low ~26 % bookings, ~25 % shows, ~16 % closes, ~15 % revenue). Cause: the lead sweep selects by `{Día} = ''` and links Día/Mes/Anuncio in one patch; the twin sweep fills `Ad` on form/manual rows *after* they were day-linked, so they never re-entered the filter — and those are the rows where staff mark results. Fix: second pass in `linkLeadsSweep` (`adUnlinkedLeadsFormula` + pure `leadAdLinkPatch`, writes only `Anuncio`, never overwrites, shares the 40/tick cap, +1 list per tick, fails soft). `POST /admin/api/metrics/sweep {"target":"leads"}` now also reports `adLinked`. Per-ad numbers read before this date (incl. §11's per-ad-set table and the 09-21 deep dive) are low for ads with many form bookings; Días/Meses were never affected (they link by day, not by ad). Not yet verified against live rows — no Airtable access in the fixing session.
