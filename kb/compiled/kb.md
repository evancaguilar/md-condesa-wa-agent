...
<!--
  kb/intake.md — Precios y políticas (rellenado a mano por Evan)

  Este archivo es la ÚNICA fuente de precios/políticas del bot. El compilador
  (tools/compile-kb.mjs) lo inserta VERBATIM en la sección "Precios y políticas"
  del KB. El horario, el contacto y las disciplinas se generan solos desde el
  sitio; NO los pongas aquí.

  Reglas:
  - Si el bot no debe inventar un dato, y aquí no está, DÉJALO como TODO. El
    prompt le prohíbe inventar precios; ante un hueco, ofrece la clase de prueba
    o pasa a un humano.
  - Precios en MXN. Edita libremente el texto; se copia tal cual al KB.
  - Tras editar, corre `node tools/compile-kb.mjs` y commitea kb/compiled/kb.md.
-->

## Planes de adultos (precio semanal, pago domiciliado)

Una sola membresía da acceso a TODAS las disciplinas de adultos (Jiu-Jitsu, Muay Thai, MMA y Box).

| Plan | Semanal | Clases | Incluye |
|---|---|---|---|
...
**Promoción vigente:** la inscripción de $999 es GRATIS al inscribirse en línea.

### Cobro cada 4 semanas — domiciliado vs. sin domiciliar

El precio de arriba es con pago **domiciliado** (cargo automático). Sin domiciliar sube:
