...
- Bulk sends / blasts (docs/blasts.md): owner-only `/admin/api/blast/*` + dashboard tab Envíos; rows = `followups` kind `blast`, run registry in kv `blast_run:<id>`; drained 6/tick by src/cron/blasts.ts inside 09:00–21:00 CDMX under a daily cap, auto-pauses on template errors. Queue refuses templates Meta has not APPROVED (needs `WA_WABA_ID`).
...
- Admin dashboard `/admin` (src/ui/admin.html, single inline-script SPA): Inicio, Chats, Aprobaciones, KB overlay, Editor (IA proposes KB edits/campaigns/rules), Campañas (+ Reglas), Envíos (owner-only blasts), Probar sandbox.
